...
In 2024–2025, voice-first and agentic applications crossed a threshold: instead of forcing users to read static tooltips or bounce between a YouTube video and your app, a voice agent can guide them in place. By pairing a UI tour engine (Joyride) with a voice layer  (Vapi Web Client) and lightweight analytics (Make), you can ship self-guided onboarding that feels like a product demo welcoming users in.
NOTE: The current SDK implementation has not fully developed the Agentic layer but merely highlights the revolutionizing value of combining Agentic apps with voice AI for product onboarding.
This guide walks through a practical approach to build a voice-guided tour that:
- Explains features conversationally
- Navigates tutorial steps hands-free using a Custom Vapi Tool ("NextStep")
- Streams per-step UI context to the Vapi voice assistant
- Captures call logs and transcripts for sentiment and insight analysis via VAPI call logs and Make


## What is a voice-guided tour?

A voice-guided tour merges a UI spotlight system (Joyride) with a voice assistant (VAPI). Joyride controls step visuals (targets, spotlight), while VAPI streams guidance and listens for commands. Each time a step changes, the app sends a short system message (selector + docs) to the assistant. The assistant explains the current feature and can move on to next tutorial steps via a Custom Vapi Tool.
...
## How Joyride and WAPI integrate
...
Example Custom Tool schema (conceptual) used by your VAPI assistant configuration:
...
![Voice Tour Flow](./assets/voice-tour-flow.png)
![Voice Tour Architecture](./assets/voice-tour-architecture.png)
...
- Join Make analytics with WAPI call logs + app telemetry for deeper funnels
- Explore agentic flows: chain tools to perform multi-step tasks on request

## Conclusion

The  voice-tour-sdk layer demonstrates how to turn static onboarding tours into conversational, agentic onboarding using Joyride + WAPI + Make. Define steps, bind a callback, stream context, and let a Custom Tool advance the UI. With call logs, transcripts, and buffered analytics, you can ship voice-first onboarding that teaches users faster—without more screen time.
