...
The voice-tour-sdk built on top of Vapi aims to work as an independent SDK implementation that can be used on top of any browser to quickly add Voice-powered agentic Product onboarding to it. 

NOTE: The current SDK implementation has not fully developed the Agentic layer but merely highlights the momentous value of combining Agentic flows with API-rich voice AI for product onboarding, and automated feedback gathering.
